...
// --------------------------------------------------
// 统计页面图标数据
CmsTiyuanFed.StatisticsDetailsData = null;
CmsTiyuanFed.newData = {};

// 留言统计初始化
CmsTiyuanFed.initStatisticsDetails = function() {
	// 初始化图表
	CmsTiyuanFed.initStatisticsDetailsData();
	// 留言按钮绑定
	this.statisticsAttachEventFun();
};


CmsTiyuanFed.statisticsAttachEventFun = function() {

	// 初始显示最先一个
	$(".cms-collapse > .panel:eq(0)").addClass('on panel-primary').children(".panel-heading").data("init", "true");
	var firstid = $(".cms-collapse > .panel:eq(0)>.panel-heading").data("key");
	CmsTiyuanFed.setHighchartsFun("chart" + firstid, CmsTiyuanFed.newData[firstid]);

	// 事件绑定
	$(".cms-collapse").on("click", '.panel-heading', function() {
		var $panel = $(this).parent(".panel"),
			isOn = $panel.hasClass('on'),
			$heading = $(".panel-heading", $panel),
			init = $heading.data("init");
		if (isOn) {
			$panel.removeClass('on panel-primary');
			$(".panel-body", $panel).hide();
		} else {
			$panel.addClass('on panel-primary').siblings('.panel').removeClass('on panel-primary');
			$(".panel-body", $panel).show();
			//  如果没有加载过图表的，加载图表
			if (init.toString() === "false") {
				var key = $heading.data("key");
				$heading.data("init", "true");
				CmsTiyuanFed.setHighchartsFun("chart" + key, CmsTiyuanFed.newData[key]);
			}
		}
	});
};
...
// 初始化图表
CmsTiyuanFed.initStatisticsDetailsData = function() {
	var obj = CmsTiyuanFed.StatisticsDetailsData;

	$.each(obj, function(i, m) {
		CmsTiyuanFed.newData[m.id] = m;
	});
};

// 初始化图表方法
CmsTiyuanFed.setHighchartsFun = function(divId, obj) {

	var rows = obj.options.length,
		colors = Highcharts.getOptions().colors,
		categories = [],
		name = "填写总人数:" + obj.total + "人",
		data = [],
		Title = "";
	for (var i = 0; i < rows; i++) {
		categories[i] = obj.options[i].name;
		data[i] = {
			'y': parseFloat((obj.options[i].count / obj.total * 100).toFixed(2)),
			'color': colors[i],
			'n': obj.options[i].count
		};
	}
